...
export default function DashboardPage() {
  ...
  const search = async (e) => {
    ...
    setLoading(true);
    try {
      const { data } = await api.post('/api/search', { ...form, passengers: party.adults + party.children });
      setResults(data);
      setIsDemo(false);
    } catch (err) {
  ...
  const applyRoute = (route) => setForm({ ...form, origin: route.from, destination: route.to });
  const handleBook = () => { /* wired once backend session is live */ };

  return (
    <div className="max-w-6xl mx-auto">
      <div className="mb-7">
        <h1 className="text-2xl font-bold text-white">Command Dashboard</h1>
        <p className="text-mist text-sm mt-1.5">Search once, compare every mode of transport at once.</p>
      </div>
      ...
      <div className="grid grid-cols-[1fr_280px] gap-4 mb-8 max-lg:grid-cols-1">
        ...
        <PartyPicker adults={party.adults} children={party.children} onChange={setParty} />
      </div>

      <div className="mb-8">
        <PersonaCard persona={persona.persona} confidence={persona.confidence} insight={persona.insight} />
      </div>

      <section className="mb-9">
        <h3 className="text-white font-semibold text-sm mb-3.5">Popular routes</h3>
        <div className="flex gap-3 overflow-x-auto pb-1">
          {POPULAR_ROUTES.map((r, i) => (
            ...
            >
              <img src={r.photo} alt={`${r.from} to ${r.to}`} loading="lazy" className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-105" />
              <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/5 to-transparent" />
              <span className="absolute top-2.5 left-2.5 text-[0.68rem] font-bold text-white px-2 py-1 rounded-full" style={{ background: r.accent }}>
                from ₹{r.priceFrom}
              </span>
              <span className="absolute bottom-3 left-3.5 right-3 text-white font-semibold text-sm text-left">
                {r.from} <span className="text-white/60 font-normal">→</span> {r.to}
              </span>
            </button>
          ))}
        </div>
      ...
      <section className="mb-9">
        <h3 className="text-white font-semibold text-sm mb-3.5">Explore India</h3>
      ...
      <section>
        ...
        <ResultsBoard options={results} onBook={handleBook} personalized={isDemo} />
      </section>
    </div>
  );
}
