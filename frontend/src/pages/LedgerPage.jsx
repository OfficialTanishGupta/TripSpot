  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="flex items-start justify-between gap-4 mb-5">
        <div>
          <h1 className="text-2xl font-bold text-ink">Data Workspace</h1>
          <p className="text-mist text-sm mt-1.5">
            Every booking you've made, all in one ledger.
          </p>
        </div>
        {isDemo && (
          <span className="text-xs font-semibold text-amber bg-amber-soft px-3 py-1 rounded-full whitespace-nowrap">
            Sample data
          </span>
        )}
      </div>
      ...
      {filtered.length === 0 ? (
        <div className="p-12 text-center text-mist-soft border border-dashed border-line rounded-2xl">
            ...
            return (
              ...
              >
                <span
                  ...
                  style={{ background: MODE_COLOR[b.mode] }}
                >
                  <Icon size={18} />
                </span>
                <div className="flex-1 min-w-0">
                  <div className="font-semibold text-ink text-sm">
                    {b.origin} <span className="text-mist-soft">→</span>{" "}
                    {b.destination}
                  </div>
                  <div className="text-xs text-mist-soft font-mono mt-0.5">
                    {b.providerName} · {b.departureTime}
                  </div>
                </div>
